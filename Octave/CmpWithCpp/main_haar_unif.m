...
Bmin = min(min(B))
Bmax = max(max(B))
figure(2);imagesc(B,[Bmin,Bmax]);title('Qudrants');%axis equal;axis([1 dim1 1 dim2]);
...
[BUQ,bmax] = unif_quant(B,th,bits,1);
b=find(BUQ==0);mb=length(b);
BU = unif_dquant(BUQ,bmax,th,bits,1);
...
AUmin = min(min(AU))
AUmax = max(max(AU))
figure(3);imagesc(AU,[AUmin,AUmax]);colormap gray(256);title('Uniform Quant');%axis equal;axis([1 dim1 1 dim2]);
